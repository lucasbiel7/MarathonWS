 ...
 * @author OCTI01
 */
public class WantBeARunnerController implements Initializable {
    ...
    @FXML
    private void btIHaveCompetedBeforeActionEvent(ActionEvent actionEvent) {
        FxMananger.inserirPainel((ScrollPane) apPrincipal.getParent().getParent().getParent(), GerenciarJanela.Janela.LOGIN);
    }

    @FXML
    private void btNewCompetitorActionEvent(ActionEvent actionEvent) {
        FxMananger.inserirPainel((ScrollPane) apPrincipal.getParent().getParent().getParent(), GerenciarJanela.Janela.REGISTERRUNNER);
    }
